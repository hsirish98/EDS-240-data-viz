...
It's important to make a clear message before you get into aesthetics

Histogram from the end of lecture 1:

Aims to ask -- does flipper length vary by penguin species? Answer: yes

**Clarity and Aesthetic Matter**
...
-   troubleshooting -- clarity will help you go back and find an error

-   note taking -- without taking notes, you do not always remember

**Live Coding Part of Discussion**
...
Regardless of where you choose to annotate, you must leave enough space to make it clear what the annotation applies to
...
##clean up dates ----
mutate(job_start_date = str_remove(job_start_date, "AM")) %>% # removed the string "AM" from the job_start_date column cells.
mutate(datetime_start = mdy_hms(job_start_date)) %>% # created a new column called "datetime_start" which contains the "job_start_date" in MM-DD-YYYY HH:MM:SS format
mutate(year = year(datetime_start)) %>% ##create a new column called "year" from column "datetime_start" that extracts just the year

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) %>% ##select relevant columns named here
  

  # filter out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) %>% ## filter out any state that doesn't have a state name
